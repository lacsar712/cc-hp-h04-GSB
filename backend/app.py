...
from order_skew import list_order_sql, latest_order_sql, present_rows, pick_latest
...
def latest_for_herb(herb: str, _user: dict = Depends(current_user)):
    with connect() as conn:
        rows = conn.execute(
            f"SELECT id, herb, doc, verdict, reason, created_by FROM batches WHERE herb=%s ORDER BY id {latest_order_sql()}",
            (herb,),
        ).fetchall()
